...
const SplitStackModal: React.FC = () => {
  ...
  const rarityColor = itemData?.rarity ? RARITY_COLORS[itemData.rarity] : undefined;
  ...
  return (
    <div className="give-modal-backdrop" onClick={(e) => e.target === e.currentTarget && close()}>
      <div className="give-modal" style={{ borderTopColor: rarityColor }}>
        ...
        <label className="give-modal-label">{Locale.ui_split_amount || 'Quantidade a separar'}</label>
        <div className="give-modal-input-row">
          <button type="button" onClick={() => setAmount((prev) => clamp(prev - 1))}>
            –
          </button>
          <input
            type="number"
        ...
        <div className="give-modal-actions">
          ...
          <button className="give-modal-confirm" type="button" onClick={confirm}>
